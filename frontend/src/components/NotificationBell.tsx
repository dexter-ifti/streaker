        ...
        <div className="relative" ref={popoverRef}>
            ...
            <button
                ...
                className={`relative p-2 rounded-xl transition-all duration-200 ${showDot
                    ? 'text-brand-punch hover:bg-brand-punch/10'
                    : 'text-ink-muted hover:text-brand-punch hover:bg-brand-lilac'
                    }`}
            >
                ...
                {/* Red dot indicator */}
                {showDot && (
                    <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-brand-punch rounded-full ring-2 ring-brand-blush animate-pulse" />
                )}
            </button>
            ...
            {open && (
                <div className="absolute right-0 top-full mt-2 w-72 z-[200] animate-fade-in-up">
                    <div className="rounded-2xl bg-surface border border-brand-orchid/80 shadow-2xl overflow-hidden">
                        {/* Popover header */}
                        <div className="flex items-center justify-between px-4 py-3 border-b border-brand-orchid/70">
                            <span className="text-ink font-semibold text-sm">Streak Reminder</span>
                            <button
                                onClick={() => setOpen(false)}
                                className="text-ink-muted hover:text-brand-punch transition-colors"
                            >
                                <X className="w-4 h-4" />
                            </button>
                        </div>
                        ...
                        <div className="p-4 space-y-3">
                            {/* Streak badge */}
                            {streak > 0 && (
                                <div className="flex items-center gap-2 px-3 py-2 rounded-xl bg-brand-blush border border-brand-orchid/70">
                                    <Flame className="w-5 h-5 text-brand-punch flex-shrink-0" />
                                    <span className="text-ink font-semibold text-sm">{streak}-day streak active</span>
                                </div>
                            )}
                            ...
                            {!hasActivityToday && streak > 0 ? (
                                <p className="text-ink text-sm leading-relaxed">
                                    ...
                                    <strong className="text-brand-punch">{streak}-day streak</strong>!
                                </p>
                            ) : hasActivityToday ? (
                                <p className="text-brand-punch text-sm">
                            ...
                            {typeof Notification !== 'undefined' && Notification.permission === 'denied' && (
                                <p className="text-brand-punch text-xs mt-2">
